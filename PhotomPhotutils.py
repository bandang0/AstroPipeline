...
df = df [(df ['x_0'] - 1150) ** 2 + (df ['y_0'] - 800) ** 2 > 250 ** 2]

result_tabBV = result_tabBV.from_pandas (df)

psf_model = DiscretePRF.create_from_image (imageSumBV - MMMBackground ().calc_background (imageSumBV), result_tabBV, 2 * fwhm - 1, mode='median')
...
result_tabBV = photometry (image = imageSumBV)
residual_imageBV = photometry.get_residual_image ()
result_tabBV.write ('result_tabBV3.2.5s.dat', format = 'ascii')
np.array (residual_imageBV).tofile ('residual_imageBV3.2.5s.dat')
...
photometry = IterativelySubtractedPSFPhotometry (finder = iraffind,
                                                ...
                                                psf_model = psf_model,
                                                fitter = LevMarLSQFitter(),
                                                niters = 2, fitshape = (2*fwhm - 1, 2*fwhm - 1),
                                                aperture_radius = fwhm)
#result_tabB = photometry (image = imageB, init_guesses = result_tabBV)
result_tabB = photometry (image = imageB, init_guesses = result_tabBV)
residual_imageB = photometry.get_residual_image ()
result_tabB.write ('result_tabB3.2.5s.dat', format = 'ascii')
np.array (residual_imageB).tofile ('residual_imageB3.2.5s.dat')
...
result_tabV.write ('result_tabV3.2.5s.dat', format = 'ascii')
np.array (residual_imageV).tofile ('residual_imageV3.2.5s.dat')
